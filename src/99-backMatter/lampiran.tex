%-----------------------------------------------------------------------------%
\addappendix{Prompts untuk Ablasi}
\begin{flushright}
    Lampiran 1: \textit{List of Prompts}
\end{flushright}
\label{appendix:ablation_prompts}
% -----------------------------------------------------------------------------%

\textit{Prompts} yang digunakan mirip dengan \textit{naive prompting explanation before answer} yang merupakan implementasi dari \textit{Chain-of-Thought}

\textit{Prompt} untuk mendapatkan jawaban untuk ablasi ~\ref{sec:fol_and_cnf_ablation}:
\lstinputlisting[caption=Template prompt untuk mencari jawaban dari SLM untuk tahapan \textit{decomposition to CNF} tanpa \textit{search and resolve}, label={lst:fol_and_cnf_prompt}]{assets/texts/decompositions_only_prompting.txt}

\textit{Prompt} untuk mendapatkan jawaban untuk ablasi ~\ref{sec:fol_ablation}:
\lstinputlisting[caption=Template prompt untuk mencari jawaban dari SLM untuk hanya dari tahapan \textit{translation to FOL}, label={lst:fol_prompt}]{assets/texts/translations_only_prompting.txt}

%-----------------------------------------------------------------------------%
\addappendix{Prolog}
\begin{flushright}
    Lampiran 2: Contoh hasil konversi ke Prolog
\end{flushright}
\label{appendix:ablation_prompts}
% -----------------------------------------------------------------------------%

Berikut adalah salah satu hasil konversi dari FOL ke bentuk yang bisa di-\textit{resolve} oleh Prolog:
\textit{Data point} dengan bentuk FOL:
\lstinputlisting[caption=\textit{Data point} dari tahapan \textit{translation to FOL}, label={lst:fol_datapoint}]{assets/codes/ProntoQA_1.json}
\textit{Prolog Form} hasil konversi:
\lstinputlisting[caption=Hasil konversi dari tahapan \textit{translation to FOL}, label={lst:fol_prolog}]{assets/codes/ProntoQA_1.pl}
