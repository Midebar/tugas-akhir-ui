...
\begin{enumerate}
    \item \textbf{Qwen2.5-7B-Instruct (\textit{multilingual foundation model} sebagai Representasi Global):} Model ini dipilih karena performanya yang unggul dalam benchmark logika matematika global, menjadi titik ukur batas atas kemampuan model \textit{open-weight} saat ini.
    \item \textbf{SEALION-v3-Llama-8B-Instruct (\textit{language-adaptive pre-trained model} sebagai Representasi Regional):} Model yang telah melalui \textit{continued pre-training} dengan data bahasa dari negara-negara di Asia Tenggara, diharapkan memiliki pemahaman semantik yang lebih baik terhadap struktur kalimat regional.
    \item \textbf{SahabatAI-v1-Llama-8B-Instruct (\textit{localized pre-trained model} sebagai Representasi Nasional):} Model yang dikhususkan untuk Bahasa Indonesia, juga sudah melalui \textit{continued pre-training}, digunakan untuk melihat apakah spesialisasi bahasa yang mendalam dapat menyelesaikan penalaran lebih baik dalam tugas logika.
\end{enumerate}
...
Setiap model dijalankan dengan pengaturan konfigurasi / parameter yang sama / konsisten untuk memastikan hasil eksperimen mencerminkan pengaruh variabel bebas, bukan perbedaan konfigurasi teknis. Strategi deterministik diterapkan untuk menghilangkan unsur \textit{randomness} dalam proses inferensi.
...
\end{figure}
Penelitian ini mengimplementasikan \textit{framework} \textbf{Aristotle} (\cite{Aristotle25}) yang membagi proses inferensi menjadi empat modul utama atau tahapan utama, sesuai dengan Gambar ~\ref{fig:aristotle_framework}, yaitu \textit{Translation to First Order Logic}, \textit{Decomposition into Conjunctive Normal Form}, \textit{Search Router}, dan \textit{Resolver}. Untuk modul / tahapan \textit{Search Router} dan \textit{Resolver} digabung menjadi satu, yaitu menjadi \textit{Search and Resolve} karena mereka saling bersinergi dalam proses resolusi
...
\begin{itemize}
    \item \textbf{Tugas Model:} Menerjemahkan narasi bahasa alami menjadi representasi \textit{First-Order Logic} (FOL) yang terdiri dari Fakta, Aturan Implikasi, dan Konjektur.
...
\begin{itemize}
    ...
    \item \textbf{Mekanisme Eksekusi:}
          \begin{itemize}
              ...
              \item \textbf{Skolemisasi:} Penghapusan kuantor eksistensial ($\exists x$) dengan mengganti variabel eksistensial dengan fungsi Skolem. Dataset ProntoQA tidak memerlukan langkah ini.
...
\subsection{Parsing Bertingkat (\textit{Multi-stage Parsing})}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.0cm}|X|X|X|}
        \hline
        \textbf{Kondisi (Skenario)} & \textbf{Jalur A: Buktikan $S$ (Cari Kontradiksi di $\neg S$)} & \textbf{Jalur B: Buktikan $\neg S$ (Cari Kontradiksi di $S$)} & \textbf{Kesimpulan}         \\
        \hline
        \centering{1}               & \textbf{Berhasil} (Ada Kontradiksi)                           & Gagal                                                         & \textbf{TRUE}               \\
        \hline
        \centering{2}               & Gagal                                                         & \textbf{Berhasil} (Ada Kontradiksi)                           & \textbf{FALSE}              \\
        \hline
        \centering{3}               & Gagal                                                         & Gagal                                                         & \textbf{UNKNOWN}            \\
        \hline
        \centering{4}               & \textbf{Berhasil}                                             & \textbf{Berhasil}                                             & \textbf{SELF-CONTRADICTORY} \\
        \hline
    \end{tabularx}
\end{table}
...
\section{Ilustrasi Proses Resolusi}
Untuk memperjelas proses resolusi menggunakan kerangka kerja \textit{framework} Aristotle, berikut adalah ilustrasi langkah demi langkah sebagai contoh dari dataset ProntoQA.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{assets/pics/Framework_steps.drawio.png}
    \captionof{figure}{Tahapan yang dilakukan dalam menjalankan \textit{framework Aristotle}}
    \label{fig:aristotle_framework_steps}
\end{figure}

\subsection{Translasi Dataset ke Bahasa Indonesia}
Sebelum dataset ProntoQA dijalankan dalam \textit{framework Aristotle}, dilakukan translasi ke bahasa Indonensia karena dataset tersebut masih dalam bahasa Inggris. Dataset ProntoQA terdiri dari konteks / premis, pertanyaan, jawaban, dan penjelasan untuk mendapatkan jawaban tersebut. Jawaban hanya mempunyai dua nilai, yaitu \textit{True / False}, dan jawaban tidak perlu ditranslasikan ke bahasa Indonesia untuk keperluan tahap evaluasi nanti.
Prosesnya sebagai berikut:
\begin{itemize}
    \item \textbf{Premis original:} "\textit{Wumpuses are sweet. Every Wumpus is jompus. All jompus are fruity. Max is wumpus.}"
    \item \textbf{Premis hasil translasi:} "Wumpuses adalah manis. Setiap Wumpus adalah jompus. Semua jompus berbuah. Max adalah wumpus."
    \item \textbf{Pertanyaan original:} "\textit{Is the following statement true or false? Max is fruity.}"
    \item \textbf{Pertanyaan hasil translasi:} "Apakah pernyataan berikut benar atau salah? Max berbuah."
    \item \textbf{Penjelasan original:} "\textit{Max is wumpus. Every wumpus is jompus. Max is jompus. All jompus are fruity. Max is fruity.}"
    \item \textbf{Penjelasan hasil translasi:} "Max adalah wumpus. Setiap wumpus adalah jompus. Max adalah jompus. Semua jompus berbuah. Max berbuah."
\end{itemize}

\subsection{\textit{Translation to FOL}}
Misalkan diberikan premis berikut dalam Bahasa Indonesia:
\begin{itemize}
    \item \textbf{Premis:} "Wumpuses adalah manis. Setiap Wumpus adalah jompus. Semua jompus berbuah. Max adalah wumpus."
    \item \textbf{Pertanyaan:} "Apakah pernyataan berikut benar atau salah? Max asam."
\end{itemize}
Model SLM menerima prompt yang berisi premis dan pertanyaan di atas, dan menghasilkan representasi FOL sebagai berikut:
\begin{itemize}
    \item \textbf{Fakta:}
          \begin{itemize}
              \item $\bracket{Wumpus}{(Max, True)}$
          \end{itemize}
    \item \textbf{Aturan:}
          \begin{itemize}
              \item $\bracket{Wumpus}{(x, True)} >>> \bracket{Manis}{(x, True)}$
              \item $\bracket{Wumpus}{(x, True)} >>> \bracket{Jompus}{(x, True)}$
              \item $\bracket{Jompus}{(x, True)} >>> \bracket{Berbuah}{(x, True)}$
          \end{itemize}
    \item \textbf{Konjektur:}
          \begin{itemize}
              \item $\bracket{Berbuah}{(Max, False)}$
          \end{itemize}
\end{itemize}

\subsection{\textit{Decomposition to CNF}}
Output aturan FOL dari tahap sebelumnya kemudian diproses untuk konversi ke CNF.
\begin{itemize}
    \item \textbf{Aturan dalam CNF:}
          \begin{enumerate}
              \item $\bracket{Wumpus}{(x, False)} \lor \bracket{Manis}{(x, True)}$
              \item $\bracket{Wumpus}{(x, False)} \lor \bracket{Jompus}{(x, True)}$
              \item $\bracket{Jompus}{(x, False)} \lor \bracket{Berbuah}{(x, True)}$
          \end{enumerate}
\end{itemize}

\subsection{\textit{Logical Resolution}}
Terakhir, modul \textit{Logical Resolver} mencoba membuktikan kebenaran konjektur $\bracket{Berbuah}{(Max, False)}$ melalui dua jalur paralel:
\begin{itemize}
    \item \textbf{Jalur A (Buktikan $S$):} Asumsikan $\bracket{Berbuah}{(Max, True)}$
          \begin{itemize}
              \item Step 1: Dari $\bracket{Berbuah}{(Max, True)}$ tidak ditemukan klausa yang berkomplemen terhadap premis yang ada.
          \end{itemize}
          Dari step tersebut, tidak ditemukan kontradiksi, sehingga Jalur A gagal membuktikan $S$.
    \item \textbf{Jalur B (Buktikan $\neg S$):} Asumsikan $\bracket{Berbuah}{(Max, False)}$
          \begin{itemize}
              \item Step 1: Dari $\bracket{Berbuah}{(Max, False)}$ dan aturan $\bracket{Jompus}{(x, False)} \lor \bracket{Berbuah}{(x, True)}$, dapat disimpulkan $\bracket{Jompus}{(Max, False)}$.
              \item Step 2: Dari $\bracket{Jompus}{(Max, False)}$ dan aturan $\bracket{Wumpus}{(x, False)} \lor \bracket{Jompus}{(x, True)}$, dapat disimpulkan $\bracket{Wumpus}{(Max, False)}$.
              \item Step 3: Dari $\bracket{Wumpus}{(Max, False)}$ dan fakta $\bracket{Wumpus}{(Max, True)}$, ditemukan klausa kosong (kontradiksi).
          \end{itemize}
          Dari step tersebut, ditemukan kontradiksi, sehingga Jalur B berhasil membuktikan $\neg S$.
\end{itemize}
Berdasarkan matriks keputusan, karena Jalur A gagal dan Jalur B berhasil, kesimpulan akhir adalah \textbf{\textit{False}}. Dengan demikian, model berhasil menyimpulkan bahwa Max berbuah berdasarkan premis yang diberikan yang bertentangan dengan konjektur awal, yaitu "Apakah Max tidak berbuah?"