...
\todo{
	Perbaiki bagian \emph{scope} dan \emph{metodologi} sesuai input data dan eksperimen aktual Anda.
}
...
\label{sec:latBel}
%-----------------------------------------------------------------------------%
Perkembangan Model Bahasa Besar (Large Language Models, LLM) dalam beberapa tahun terakhir telah menunjukkan kemampuan mengesankan pada berbagai tugas pemrosesan bahasa alami seperti penerjemahan, ringkasan, dan tanya-jawab. Salah satu aspek penting yang menjadi fokus kajian ilmiah adalah kemampuan \emph{penalaran logis} — kemampuan menarik inferensi yang benar dari kumpulan premis dan aturan. 

Sebagian besar benchmark penalaran dan reasoning (misalnya ProofWriter, LogicNLI, ProntoQA) ditujukan pada bahasa Inggris. Namun demikian, studi terhadap kemampuan penalaran LLM pada bahasa lain, termasuk Bahasa Indonesia, masih terbatas. Perbedaan struktur linguistik, kebijakan tanda baca, penggunaan kata majemuk, serta isu translasi otomatis dapat memengaruhi performa model ketika berpindah domain bahasa.

Penelitian ini bertujuan untuk mengevaluasi kemampuan penalaran logis LLM pada dataset berbahasa Indonesia dan menguji efektivitas pipeline reasoning berbasis modul: translasi/normalisasi, dekomposisi aturan, pencarian klausa komplement, serta resolusi logika. Fokus utama adalah mendapatkan gambaran kuantitatif dan kualitatif tentang batas kemampuan LLM pada skenario berbahasa Indonesia dan menyajikan rekomendasi praktis untuk eksperimen selanjutnya.
...
\begin{enumerate}
	\item Apakah LLM dapat melakukan penalaran logis yang konsisten pada dataset berbahasa Indonesia?
	\item Bagaimana perbandingan performa antara model lokal (open-source, kuantisasi) dan model pembanding (API/remote) pada tugas penalaran?
	\item Seberapa efektif pipeline berbasis dekomposisi dan pencarian klausa komplement dalam meningkatkan akurasi penalaran?
	\item Apa hambatan utama yang muncul saat memindahkan dataset penalaran dari bahasa Inggris ke Bahasa Indonesia (mis. tokenisasi, struktur kalimat, negasi)?
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Tujuan khusus:}
	      \begin{enumerate}
		      ...
		      \item Mengidentifikasi sumber kesalahan (error analysis) dan memberikan rekomendasi praktis.
	      \end{enumerate}
\end{itemize}
...
\begin{itemize}
	...
	\item Evaluasi: metrik utama adalah akurasi final (A/B/C/D sesuai pengkodean), dan analisis kualitatif terhadap kasus salah.
	\item Lingkup eksperimen dibatasi pada dev/test split dataset yang telah disiapkan.
\end{itemize}
...
\begin{itemize}
	...
	\item Panduan teknis untuk penelitian lanjutan & praktisi (pemilihan model, pipeline, penanganan negasi).
	\item Dataset dan skrip eksperimen yang dapat direplikasi oleh komunitas.
\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Backend LLM:} Konfigurasi backend model (local HF / kuantisasi / shim API) dan tokenizers, memastikan konsistensi pad/eos token.
	\item \textbf{Pipeline reasoning:} Implementasi modul:
	      \begin{itemize}
		      ...
		      \item \texttt{search\_resolve.py} — pencarian klausa komplement & logic resolution,
		      \item \texttt{evaluate.py} — perhitungan akurasi & laporan.
	      \end{itemize}
	\item \textbf{Eksperimen:} Jalankan pengujian pilot, ukur akurasi, performa (waktu, memori), dan analisis kesalahan.
	\item \textbf{Analisis:} Klasifikasi kesalahan, bandingkan per-model dan per-pipeline, serta tarik rekomendasi.
\end{enumerate}
