...
Sintaks FOL dibangun dari komponen-komponen berikut menurut \cite[Chapter 2]{brachman2004knowledge}: 
\begin{itemize}
	\item Simbol Logis:
	      \begin{itemize}
...
%-----------------------------------------------------------------------------%
Metode inferensi utama yang digunakan dalam modul \textit{resolve} Aristotle adalah \textbf{Resolusi} \cite[Chapter 4]{brachman2004knowledge}
...
Tanda terjadinya kontradiksi adalah ketika algoritma berhasil menurunkan \textbf{Klausa Kosong} ($\square$ atau \textit{False}) \cite[Chapter 4]{brachman2004knowledge}.
...
\begin{itemize}
	...
	\item \textbf{(\textit{Edge Deployability}):} Karena ukurannya yang ringkas, SLM dapat dijalankan secara lokal pada perangkat konsumen standar, seperti laptop, ponsel, tanpa memerlukan kluster GPU \textit{data center}. Hal ini memberikan keuntungan dalam privasi data dan kedaulatan informasi (\cite{belcak2025smalllanguagemodelsfuture}).
	\item \textbf{Teknik Kompresi:} Performa tinggi SLM sering kali dicapai melalui teknik \textit{Knowledge Distillation} (belajar dari model guru yang lebih besar) dan pelatihan pada dataset berkualitas sangat tinggi ("textbook quality"), serta penggunaan teknik pemangkasan (\textit{pruning}) (\cite{10.1145/3768165}).
\end{itemize}
Model seperti Microsoft Phi-3, Mistral 7B, dan Gemma 2B adalah contoh representatif dari kelas model ini yang menunjukkan kemampuan penalaran mengejutkan meskipun berukuran kecil ("Tiny but Mighty")\footnote{\url{https://news.microsoft.com/source/features/ai/the-phi-3-small-language-models-with-big-potential/}}
...
\begin{enumerate}
	\item Sistem membangkitkan graf ontologi fiktif (misal: $Wumpus \rightarrow Jompus \rightarrow Tumpus$).
