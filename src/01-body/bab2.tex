...
\begin{itemize}
	...
	\item Simbol Non-Logis:
	      \begin{itemize}
		      ...
		      \item Predikat ($P$($x_1$, $\dots$, $x_n$)): Fungsi yang memetakan \textit{tuple} objek ke nilai kebenaran (True/False). Contoh: $Ayah$($Budi, True$).
	      \end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Fungsi:} Setiap simbol fungsi $f$ dipetakan ke operasi konkret pada $\mathcal{D}$. Misalnya, jika domainnya adalah bilangan bulat, simbol fungsi $\bracket{plus}{(x, y)}$ dapat diinterpretasikan sebagai operasi penjumlahan $x$ + $y$.
\end{itemize}
...
\subsection{\textit{Pre-training dan Fine-tuning}}
...
\subsection{Definisi dan Karakteristik \textit{Small Language Models} (SLM)}
Dalam perkembangannya, paradigma model bahasa mulai bergeser dari "semakin besar semakin baik", yang artinya semakin besar suatu LLM, maka semakin baik performanya, menjadi "semakin efisien semakin baik" karena \textit{unfeasible} untuk menjalankan LLM-LLM yang besar tersebut, terutama pada sistem dengan \textit{resource} yang terbatas. \textit{Small Language Models} (SLM) didefinisikan secara longgar dalam literatur sebagai model dengan jumlah parameter berkisar antara beberapa juta hingga batas atas sekitar 7 hingga 10 miliar parameter (\cite{10.1145/3768165, subramanian2025smalllanguagemodelsslms}).
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{assets/pics/ProntoQA_generation.png}
	\captionof{figure}{Tahapan yang dilakukan dalam generasi dataset ProntoQA (\cite{saparov2023language})}
	\label{fig:prontoqa_generation}
\end{figure}

Proses pembuatannya adalah:
\begin{enumerate}
	\item Sistem membuat graf ontologi fiktif (misal: $\bracket{Wumpus}{\text{Deskripsi: Merah}} \rightarrow \bracket{Jompus}{\text{Deskripsi: Transparan}}$. $\bracket{Jompus}{\text{Deskripsi: Transparan}} \rightarrow Tumpus$.)
	\item Sistem membuat \textit{proof} dari ontologi tersebut (misal: $\bracket{Wumpus}{Max}$. $\bracket{\forall x}{\bracket{Wumpus}{x}} \rightarrow \bracket{Merah}{x}$. $\bracket{\forall x}{\bracket{Wumpus}{x}} \rightarrow \bracket{Jompus}{x}$.)
	\item Simbol-simbol logika diganti dengan kata-kata fiktif atau nyata untuk membentuk kalimat bahasa alami (misal: Max adalah wumpus. Setiap wumpus adalah merah. Wumpuses adalah jompus. Jompuses bersifat transparan. Semua jompus adalah Tumpus.)
\end{enumerate}
...
Dataset ini juga memungkinkan kontrol terhadap "kedalaman penalaran" (\textit{reasoning hops}), yaitu berapa langkah logika yang diperlukan untuk mencapai kesimpulan (misal: \textit{1-hop}, \textit{3-hop}, hingga \textit{5-hop}).